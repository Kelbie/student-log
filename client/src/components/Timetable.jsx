...
function calculateDuration(classes) {
    return classes.map(class_ => {
        return { ...class_, duration: moment.unix(parseInt(class_.end) / 1000).diff(moment.unix(parseInt(class_.start) / 1000), "hour") }
    })
}
    ...
    return (
        <div {...props}>
            {[...Array(24)].map((class_, i) => {
                return <><Timelabel index={i}>{classes[0] && moment.unix(parseInt(classes[0].start) / 1000).add(i, "hours").format("HH:mm")}</Timelabel><Timebar index={i} /></>;
            })}
            {[...classes, ...gaps]
                .sort(function (a, b) {
